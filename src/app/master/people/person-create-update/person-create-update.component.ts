import { Component, Inject, OnInit } from '@angular/core';
import { MAT_DIALOG_DATA, MatDialogRef } from '@angular/material';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { Store } from '@ngrx/store';
import * as fromRoot from '../../../reducers/index';
import { Person } from './person-model';

@Component({
  selector: 'elastic-create-update',
  templateUrl: './person-create-update.component.html',
  styleUrls: ['./person-create-update.component.scss']
})
export class PersonCreateUpdateComponent implements OnInit {

  static id = 100;

  form: FormGroup;
  mode: 'create' | 'update' = 'create';

  constructor(@Inject(MAT_DIALOG_DATA) public defaults: any,
              private dialogRef: MatDialogRef<PersonCreateUpdateComponent>,
              private fb: FormBuilder,
              private store: Store<fromRoot.State>) {
  }

  ngOnInit() {
    if (this.defaults) {
      this.mode = 'update';
    } else {
      this.defaults = {} as Person;
    }

    this.form = this.fb.group({
      id: [PersonCreateUpdateComponent.id++],
      firstName: [this.defaults.firstName || '',],
      lastName: [this.defaults.lastName || ''],
      street: this.defaults.street || '',
      city: this.defaults.city || '',
      zipcode: this.defaults.zipcode || '',
      phoneNumber: this.defaults.phoneNumber || '',
    });
  }

  save() {
    if (this.mode === 'create') {
      this.createPerson();
    } else if (this.mode === 'update') {
      this.updatePerson();
    }
  }

  createPerson() {
    const person = this.form.value;
    this.dialogRef.close(person);
  }

  updatePerson() {
    const person = this.form.value;
    person.id = this.defaults.id;

    this.dialogRef.close(person);
  }

  isCreateMode() {
    return this.mode === 'create';
  }

  isUpdateMode() {
    return this.mode === 'update';
  }
}
